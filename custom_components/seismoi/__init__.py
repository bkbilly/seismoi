...
from .manager import SeismoiFeedEntityManager

_LOGGER = logging.getLogger(__name__)


async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    """Set up the Seismoi component as config entry."""
    feeds = hass.data.setdefault(DOMAIN, {})
    # Create feed entity manager for all platforms.
    manager = SeismoiFeedEntityManager(hass, config_entry)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload the Seismoi config entry."""
    unload_ok = await hass.config_entries.async_unload_platforms(entry, PLATFORMS)
    if unload_ok:
        manager: SeismoiFeedEntityManager = hass.data[DOMAIN].pop(entry.entry_id)
